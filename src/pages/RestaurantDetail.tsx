...
import { useDbRestaurantById, useDbCityById, useProducts } from '@/hooks/useProducts';
...
const RestaurantDetail = () => {
  ...
  const { restaurant, loading: restLoading } = useDbRestaurantById(restaurantId);
  const { city, loading: cityLoading } = useDbCityById(restaurant?.city_id);
  const { products: menuItems, loading: menuLoading } = useProducts({ restaurantId });
  const { products: popularItems } = useProducts({ restaurantId, popularOnly: true });
      ...
      <div className="min-h-screen bg-background">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="relative animate-fade-in">
              <div className="aspect-video rounded-2xl overflow-hidden shadow-card">
                <img src={restaurant.image_url || '/placeholder.svg'} alt={restaurant.name} className="w-full h-full object-cover" />
              </div>
              {restaurant.is_veg && (
            ...
            <div className="space-y-4 sm:space-y-6 animate-fade-in" style={{ animationDelay: '0.1s' }}>
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                <div className="flex items-center gap-1 bg-secondary px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-sm sm:text-base">
                  ...
                  <span className="text-muted-foreground text-xs sm:text-sm">({restaurant.review_count})</span>
                </div>
                <div className="flex items-center gap-1 bg-secondary px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-sm sm:text-base">
              ...
              <div className="bg-card rounded-xl p-6 shadow-card border border-border">
                <h3 className="font-display text-lg font-semibold text-foreground mb-4">Location</h3>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                ...
                <Link to={`/menu`} className="flex-1">
                  <Button variant="outline" className="w-full text-sm sm:text-base">View Full Menu</Button>
                </Link>
              </div>
            </div>
          </div>
          ...
          <div className="container mx-auto px-4 py-8 sm:py-12">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {popularItems.slice(0, 8).map((item, index) => (
                <div key={item.id} className="group bg-card rounded-xl overflow-hidden shadow-card hover:shadow-xl transition-all duration-300 hover:-translate-y-1 animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                  <Link to={`/food/${item.id}`}>
                    <div className="aspect-square overflow-hidden relative">
                      <img src={item.image} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                      <div className="absolute top-2 left-2 bg-primary text-primary-foreground px-2 py-0.5 rounded text-xs font-medium">Bestseller</div>
                    </div>
                  </Link>
                  <div className="p-2 sm:p-3">
                    <h3 className="font-medium text-foreground text-xs sm:text-sm line-clamp-1">{item.name}</h3>
                    <p className="text-xs text-muted-foreground line-clamp-1 mt-0.5 sm:mt-1 hidden sm:block">{item.description}</p>
                    ...
                    <Button size="sm" className="w-full mt-1 sm:mt-2 text-xs sm:text-sm h-7 sm:h-8" onClick={() => handleAddToCart(item)}>
        ...
        <div className="bg-secondary/30 py-8 sm:py-12">
          <div className="container mx-auto px-4">
            <h2 className="font-display text-xl sm:text-2xl font-bold text-foreground mb-4 sm:mb-6">📋 Menu</h2>
            {menuLoading ? (
              <div className="text-center py-8 text-muted-foreground">Loading menu...</div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {menuItems.map((item, index) => (
                  <div key={item.id} className="bg-card rounded-xl p-3 sm:p-4 shadow-card flex gap-3 sm:gap-4 animate-fade-in hover:shadow-lg transition-shadow" style={{ animationDelay: `${index * 0.05}s` }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="font-bold text-primary text-sm sm:text-base">₹{item.price}</span>
                        <Button size="sm" variant="outline" className="h-7 sm:h-8 text-xs sm:text-sm px-2 sm:px-3" onClick={() => handleAddToCart(item)}>Add</Button>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {city.lat && city.lng && (
          <div className="py-12">
            <div className="container mx-auto px-4">
              <h2 className="font-display text-2xl font-bold text-foreground mb-6">Restaurant Location</h2>
              <div className="rounded-xl overflow-hidden shadow-card h-[400px]">
                <iframe
                  title={`Map of ${restaurant.name}`}
                  src={`https://www.openstreetmap.org/export/embed.html?bbox=${city.lng - 0.05}%2C${city.lat - 0.03}%2C${city.lng + 0.05}%2C${city.lat + 0.03}&layer=mapnik&marker=${city.lat}%2C${city.lng}`}
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {restaurantReviews.map((review, index) => (
                  <div key={review.id} className="bg-card rounded-xl p-6 shadow-card animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                        <span className="font-semibold text-primary">{review.userName.charAt(0)}</span>
                      </div>
                      <div>
                        <p className="font-medium text-foreground">{review.userName}</p>
                        <div className="flex items-center gap-1">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} className={`w-3 h-3 ${i < review.rating ? 'text-spice-turmeric fill-spice-turmeric' : 'text-muted'}`} />
                          ))}
                        </div>
                      </div>
                    </div>
