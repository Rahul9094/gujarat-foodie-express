...
import { useProductById, useProducts, useDbRestaurantById, useDbCityById } from '@/hooks/useProducts';
...
const FoodDetail = () => {
  ...
  // Fetch from database
  const { product: food, loading: foodLoading } = useProductById(foodId);
  const { restaurant, loading: restLoading } = useDbRestaurantById(food?.restaurantId);
  const { city, loading: cityLoading } = useDbCityById(restaurant?.city_id);
  
  // Related foods from same restaurant
  const { products: relatedProducts } = useProducts({ restaurantId: food?.restaurantId });
  const relatedFoods = relatedProducts.filter(f => f.id !== foodId).slice(0, 4);

  useEffect(() => {
    const storedReviews = localStorage.getItem(`reviews_${foodId}`);
    const dbReviews = mockReviews.filter(r => r.itemId === foodId);
    if (storedReviews) {
  ...
  if (foodLoading || restLoading || cityLoading) {
    return (
      <Layout>
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-background">
            ...
            <div className="space-y-6 animate-fade-in" style={{ animationDelay: '0.1s' }}>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                <div className="flex items-center justify-center gap-3 bg-secondary rounded-xl p-2">
                  <Button variant="ghost" size="icon" onClick={() => setQuantity(Math.max(1, quantity - 1))} className="h-9 w-9 sm:h-10 sm:w-10">
                    <Minus className="w-4 h-4" />
                  </Button>
                  <span className="w-10 sm:w-12 text-center font-bold text-lg sm:text-xl">{quantity}</span>
                  <Button variant="ghost" size="icon" onClick={() => setQuantity(quantity + 1)} className="h-9 w-9 sm:h-10 sm:w-10">
                    <Plus className="w-4 h-4" />
                  </Button>
                </div>
                <Button variant="hero" size="lg" onClick={handleAddToCart} className="flex-1 text-sm sm:text-base">
                    ...
                    <div className="bg-card rounded-xl overflow-hidden shadow-card group">
                      <div className="relative h-32">
                        ...
                        <div className="absolute bottom-4 left-4">
                          ...
                          <h4 className="font-display text-xl font-bold text-background">{city.name}</h4>
                        </div>
                      </div>
                      <div className="p-4">
                        <p className="text-sm text-muted-foreground">
                          {city.restaurant_count} restaurants delivering in this city
                        </p>
                      </div>
                    </div>
                  </Link>
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} onClick={() => setReviewRating(star)} className="p-1">
                        <Star className={`w-6 h-6 transition-colors ${star <= reviewRating ? 'text-spice-turmeric fill-spice-turmeric' : 'text-muted hover:text-spice-turmeric'}`} />
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div key={review.id} className="bg-card rounded-xl p-6 shadow-card animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {review.id.startsWith('rev_') && (
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" onClick={() => handleEditReview(review)} className="text-primary hover:text-primary hover:bg-primary/10">
                          <Pencil className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteReview(review.id)} className="text-destructive hover:text-destructive hover:bg-destructive/10">
                  ...
                  {editingReview === review.id ? (
                    <div className="space-y-3 mt-3">
                        ...
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <button key={star} onClick={() => setEditRating(star)} className="p-1">
                              <Star className={`w-5 h-5 transition-colors ${star <= editRating ? 'text-spice-turmeric fill-spice-turmeric' : 'text-muted hover:text-spice-turmeric'}`} />
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" variant="hero" onClick={() => handleSaveEdit(review.id)}>Save</Button>
        ...
        {/* Related Items */}
        {relatedFoods.length > 0 && restaurant && (
          <div className="bg-secondary/30 py-12">
            <div className="container mx-auto px-4">
              <h2 className="font-display text-2xl font-bold text-foreground mb-6">
